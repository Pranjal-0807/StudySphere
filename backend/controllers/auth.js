...
exports.login = async (req, res) => {
    try {
        ...
        let user = await User.findOne({ email }).select('+password').populate('additionalDetails');
        ...
        if (await bcrypt.compare(password, user.password)) {
            const payload = {
                email: user.email,
                id: user._id,
                accountType: user.accountType // This will help to check whether user have access to route, while authorization
            };
